/* WiFi station with reconnection and deep sleep
 *
 * File: wifi_handler.c
 * Author: antusystem <aleantunes95@gmail.com>
 * Date: 26th October 2023
 * Description: Handler Wifi init, config, (re)connection, and entering deep sleep.
 * 
 * This file is subject to the terms of the Apache License, Version 2.0, January 2004.
 */

// Though the WiFi Station example have this libraries, they are no needed to compile
// #include <stdio.h>
// #include <string.h>
// #include "esp_event.h"
// #include "esp_flash.h"
...
void ESP_WIFI_Task(void *P){
    ...
    for(;;){
        ...
        if (bits & WIFI_CONNECTED_BIT) {
            ...
            // Setting bit to indicate that connection was established
            xEventGroupSetBits(
                s_wifi_event_group,
                SNTP_INIT_BIT
            );

            // Wait until the date has been updated to enter deep sleep
            xEventGroupWaitBits(
                s_wifi_event_group,
                SNTP_END_BIT,
                pdFALSE,
            ...
            // In this case the disconnection won't trigger another connection 
            // because the device will enter deep sleep before processing that
